...
def my_get_samples(config, score_model, sampling_fn):
    """cocoaaa: my procedure to sample from a ckpt of the model as nparr of npimages 
    each of which is converted from range [0.0,1.0] (the range of samples from score_model),
    to uint8_nparr in range {0, 1, ..., 255}.
     
    """
    # breakpoint()
    samples, n = sampling_fn(score_model) #samples: batch of (flattened) float tensors in range (0.0, 1.0)
    samples = np.clip(samples.permute(0, 2, 3, 1).cpu().numpy() * 255., 0, 255).astype(np.uint8)
    samples = samples.reshape((-1, config.data.image_size, config.data.image_size, config.data.num_channels))
    return samples
